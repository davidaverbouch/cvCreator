...
export const workExperienceForCv = (cvRow: CV): WorkExperience[] =>
  db
    .prepare(
      `SELECT * FROM work_experience xp JOIN cv_work_experience cs ON cs.work_experience_id = xp.id WHERE cs.cv_id = ?`
    )
    .all(cvRow.id) as WorkExperience[];

// Ajouter une expérience professionnelle
export const insertWorkExperience = ({
  title,
  year,
  duration,
  company_name,
  ...
  return db
    .prepare(
      `INSERT INTO work_experience (title, year, duration, company_name, is_esn, esn_name) VALUES (?, ?, ?, ?, ?, ?)`
    )
    .run(title, year, duration, company_name, is_esn, esn_name).lastInsertRowid;
};

// Associer une expérience professionnelle au cv
// ex: insertWorkExperienceIntoCV('David', 'Averbouch', 'Capgemini)
export const insertWorkExperienceIntoCV = ({
  firstName,
  lastName,
  companyName,
}: {
