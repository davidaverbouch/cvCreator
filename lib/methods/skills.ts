...
export const skillsForCv = (cvRow: CV): Skill[] =>
  db
    .prepare(`SELECT * FROM skills s JOIN cv_skills cs ON cs.skill_id = s.id WHERE cs.cv_id = ?`)
    .all(cvRow.id) as Skill[];

// Ajouter un skill
export const insertSkills = ({
  name,
  duration,
  level,
  isMain,
  ...
  return db
    .prepare(`INSERT INTO skills (name, duration, level, is_main, category) VALUES (?, ?, ?, ?, ?)`)
    .run(name, duration, level, isMain, category).lastInsertRowid;
};

// Associer un skill au cv
// ex: insertSkillIntoCV('David', 'Averbouch', 'Reactjs)
export const insertSkillIntoCV = ({
  firstName,
  lastName,
  skillName,
}: {
