...
export const getUser = ({ firstName, lastName }: { firstName: string; lastName: string }): User | undefined => {
  const user = db
    ...
    .get(firstName, lastName) as UserDTO;
...
export const insertUser = ({
  ...
  return db
    .prepare(
      `INSERT INTO users (sex, first_name, last_name, birthday, phone, email, address, drive_licence) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(sex, firstName, lastName, birthday.getTime(), phone, email, address, driveLicence);
};
